...
export class SecureChannel {
  ...
  async encryptAppMessage(
    ...
    const encrypted = await this.crypto.encrypt(
      encodeAppMessage(message),
      createAadContext(draft, this.profile),
    );
    return {
      ...draft,
  ...
  async decryptAppMessage(env: Envelope): Promise<AppMessage> {
    if (env.outerType !== OuterType.DATA)
      throw new Error("Expected encrypted data frame");
    const plaintext = await this.crypto.decrypt(
      env.payload,
      env.nonce,
      env.counter,
      createAadContext(env, this.profile),
    );
